...
function resolve(segments) {
  ...
  // регистрация и вход. Профиль лежит в /api/users/{id} — отдельного
  // /auth/me не держим: без обязательного токена он ничего не добавляет
  if (s.length === 2 && s[0] === 'auth') {
    return { handler: H.auth, params: { action: s[1] } };
  }
...
const KNOWN_PATHS = [
  ...
  '/api/users', '/api/users/{id}', '/api/users/{id}/avatar',
...
const RESERVED_SLUGS = Object.keys(RESERVED).concat(['auth', 'images', 'swagger', 'swagger.json']);
