...
// --- телефон ------------------------------------------------
// В публичном ответе номер всегда неполный: видны код и начало,
// последние шесть цифр закрыты. +992900123456 → +992900******
function maskPhone(tel) {
  const raw = String(tel || '');
  if (!raw) return '';
  const digits = raw.replace(/\D/g, '');
  if (digits.length <= 6) return '*'.repeat(digits.length);
  const hidden = Math.max(6, digits.length - 7);
  const visible = digits.slice(0, digits.length - hidden);
  return (raw.trim()[0] === '+' ? '+' : '') + visible + '*'.repeat(hidden);
}
