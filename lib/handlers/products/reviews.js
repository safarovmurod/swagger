...
module.exports = async (req, res) => {
  ...
  // POST /api/products/{id}/reviews — отзыв оставляет вошедший пользователь.
  // Автор берётся из профиля, а не из тела: подписаться чужим именем нельзя.
  if (req.method === 'POST') {
    ...
    if (product.reviews.some(r => r.userId === user.id)) {
      return jsonErr(res, 409, 'Вы уже оставляли отзыв на этот товар', 'REVIEW_ALREADY_EXISTS');
    }
