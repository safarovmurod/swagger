const {
  products, categories, jsonRes, jsonData, jsonErr, paginate, readBody, nextId,
  queryProducts, requireAdmin, withOwner
} = require('../_helpers');
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // если студент прислал userId, запись становится его: чужой её не удалит
    withOwner(newProduct, req, body);
