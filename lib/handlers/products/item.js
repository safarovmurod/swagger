const {
  products, categories, jsonData, jsonErr, readBody, miniProduct, requireAdmin, ownerBlocks
} = require('../_helpers');
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'PUT' || req.method === 'PATCH') {
    ...
    const denied = ownerBlocks(product, req, body);
    if (denied) return jsonErr(res, 403, denied, 'FORBIDDEN');
    const keys = Object.keys(body).filter(k => !READ_ONLY.includes(k) && k !== 'userId');
  ...
  if (req.method === 'DELETE') {
    ...
    const denied = ownerBlocks(product, req, null);
    if (denied) return jsonErr(res, 403, denied, 'FORBIDDEN');
