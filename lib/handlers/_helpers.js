...
// Гость — тот, кто пишет без входа. Это учебный тренажёр: создавать,
// менять и удалять записи можно без токена, иначе занятие превращается
// в борьбу с 401. Строгий режим включается переменной ADMIN_WRITES=true.
const GUEST = { id: 0, role: 'admin', fullName: 'Гость', email: '' };

// Токен необязателен, но если он прислан — им пользуемся
...
async function requireUser(req, res) {
  if (!config.adminWrites) return (await optionalUser(req)) || GUEST;
...
// --- «удалить может только тот, кто создал» ------------------
// Входа нет, поэтому автора записи определяем по userId, который клиент
// присылает при создании. Кто создал с userId=7, тот и меняет — с тем же
// userId. Записи из готового каталога владельца не имеют и открыты всем.
function requesterId(req, body) {
  const q = req.query || {};
  const raw = (body && body.userId !== undefined) ? body.userId : q.userId;
  const id = parseInt(raw, 10);
  return Number.isFinite(id) ? id : null;
}

// Возвращает текст ошибки, если запись чужая, и null, если можно менять
function ownerBlocks(record, req, body) {
  const owner = record && record.ownerId;
  if (owner === undefined || owner === null) return null;
  const asked = requesterId(req, body);
  if (asked !== null && asked === owner) return null;
  const who = (req._user && req._user.role === 'admin' && req._user.id) ? true : false;
  if (who) return null;
  return `Эту запись создал пользователь ${owner} — менять и удалять её может только он. ` +
    `Передайте userId=${owner} в теле запроса или в строке запроса.`;
}

// Кладём владельца в новую запись, если клиент его указал
function withOwner(record, req, body) {
  const owner = requesterId(req, body);
  if (owner !== null) record.ownerId = owner;
  return record;
}
...
module.exports = {
  ...
  authenticate, optionalUser, requireUser, requireAdmin,
  requesterId, ownerBlocks, withOwner, GUEST
};
