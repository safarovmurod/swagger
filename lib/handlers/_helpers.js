...
// Единый конверт ответа: у КАЖДОГО ответа есть поле data.
//   список  → { data: [...], totalCount, page, pageSize, totalPages, hasPrevious, hasNext }
//   объект  → { data: {...} }
//   ошибка  → { data: null, message: '...' }
// На фронтенде это даёт одинаковый разбор: const { data } = await axios.get(url) → data.data
...
function jsonErr(res, status, message) {
  return jsonRes(res, status, { data: null, message });
}
...
module.exports = {
  ...
  setCors, jsonRes, jsonData, jsonErr, paginate, getBody, lightProduct, miniProduct, nextId
};
