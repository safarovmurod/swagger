const {
  promotions, products, jsonRes, jsonData, jsonErr, paginate, readBody, miniProduct,
  requireAdmin, ownerBlocks
} = require('../_helpers');
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'PUT' || req.method === 'PATCH') {
    ...
    const denied = ownerBlocks(promo, req, body);
  ...
  if (req.method === 'DELETE') {
    ...
    const denied = ownerBlocks(promotions[idx], req, null);
    if (denied) return jsonErr(res, 403, denied, 'FORBIDDEN');
