...
const { categories, products, jsonRes, jsonData, jsonErr, paginate, lightProduct } = require('../_helpers');
...
module.exports = async (req, res) => {
  ...
  if (req.method !== 'GET') return jsonErr(res, 405, 'Метод не поддерживается');

  const cat = categories.find(c => c.slug === catKey || c.id === parseInt(catKey));
  if (!cat) return jsonErr(res, 404, `Категория «${catKey}» не найдена`);

  const sub = cat.subcategories.find(s => s.slug === subKey || s.id === parseInt(subKey));
  if (!sub) return jsonErr(res, 404, `Подкатегория «${subKey}» не найдена в категории «${cat.name}»`);
