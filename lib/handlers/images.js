...
//
// Теперь у каждого товара есть своя студийная фотография на белом фоне:
// сначала ищем assets/products/{slug}.jpg (?variant=2 — второй ракурс
// в другом цвете из assets/products/v2/). Расширение .svg в URL оставлено
// намеренно: исходные данные не меняются, поэтому ни один уже работающий
// клиент не ломается — меняется только Content-Type ответа.
// ============================================================
const fs = require('fs');
const path = require('path');
const { categories, products, blog, setCors } = require('./_helpers');

// Каталог с готовыми фотографиями товаров
const PHOTO_DIR = path.join(__dirname, '..', '..', 'assets', 'products');

// slug приходит из URL, поэтому алфавит жёстко ограничен — никаких ".."
// или слешей в имени файла оказаться не может.
const SAFE_SLUG = /^[a-z0-9-]{1,120}$/;

function photoPath(slug, variant) {
  if (!SAFE_SLUG.test(slug)) return null;
  const candidates = variant === 2
    ? [path.join(PHOTO_DIR, 'v2', slug + '.jpg'), path.join(PHOTO_DIR, slug + '.jpg')]
    : [path.join(PHOTO_DIR, slug + '.jpg')];
  for (const file of candidates) {
    try { if (fs.statSync(file).isFile()) return file; } catch { /* файла нет */ }
  }
  return null;
}
...
module.exports = async (req, res) => {
  ...
  const q = req.query || {};
  const slug = String(q.slug || '').replace(/\.(svg|jpe?g)$/i, '');
  const variant = q.variant === '2' ? 2 : 1;

  // Главный путь — реальная фотография товара
  const photo = photoPath(slug, variant);
  if (photo) {
    let body = null;
    try { body = fs.readFileSync(photo); } catch { /* читать не вышло — уйдём в SVG */ }
    if (body) {
      res.statusCode = 200;
  ...
  // Фото нет — рисуем SVG-заглушку, карточка никогда не остаётся пустой
