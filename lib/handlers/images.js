...
// ============================================================
const { categories, products, blog, setCors, setSecurityHeaders } = require('./_helpers');
...
// инициалы автора для аватара: «Наталья Гафурова» → «НГ»
...
function lookup(slug) {
  ...
  if (slug.indexOf('author-') === 0) {
    ...
    const post = blog.find(b => b.author && b.author.slug === key);
    if (post) return { title: post.author.name, sub: post.author.role, catSlug: 'blog', icon: 'author', initials: initialsOf(post.author.name) };
  }
...
module.exports = async (req, res) => {
  ...
  // Аватар автора — отдельная, более простая картинка: круг и инициалы
  if (found.initials) {
    const avatar =
`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="200" height="200" role="img" aria-label="${esc(found.title)}">
  <rect width="200" height="200" fill="#ffffff"/>
  <circle cx="100" cy="100" r="100" fill="${color}" opacity="0.14"/>
  <circle cx="100" cy="100" r="76" fill="${color}" opacity="0.18"/>
  <text x="100" y="100" text-anchor="middle" dominant-baseline="central"
        font-family="-apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif"
        font-size="64" font-weight="600" fill="${color}">${esc(found.initials)}</text>
</svg>`;
    res.statusCode = 200;
