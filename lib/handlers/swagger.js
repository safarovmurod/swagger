...
const envelope = (ref) => {
  ...
  if (/Page$/.test(ref)) return { $ref: `#/components/schemas/${ref}` };
  return { type: 'object', properties: { data: { $ref: `#/components/schemas/${ref}` } } };
};
const ok = (ref, ru, en) => ({
  ...
  res.end(JSON.stringify({
    ...
    info: {
      ...
      description:
        ...
        'Каждый ответ приходит в конверте: { data: ... }. У списка рядом с data лежат ' +
        'totalCount, page, pageSize, totalPages, hasPrevious, hasNext; у ошибки — { data: null, message }.',
      'x-en': {
        ...
        description:
          ...
          'No authentication required, CORS is open. Every response is wrapped: `{ data: ... }` — ' +
          'a list also carries totalCount, page, pageSize, totalPages, hasPrevious, hasNext next to data, ' +
          'and an error returns `{ data: null, message }`.'
      },
    ...
    components: {
      schemas: {
        Error: {
          type: 'object',
          description: 'Ошибка приходит в том же конверте: data всегда null',
          properties: {
        ...
        Ok: {
          type: 'object',
          properties: { data: { type: 'object', nullable: true, description: 'Результат операции' } }
        },
        CategoryInfo: {
          type: 'object',
