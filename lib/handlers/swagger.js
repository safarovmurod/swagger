...
module.exports = (req, res) => {
  ...
  res.end(JSON.stringify({
    ...
    tags: [
      { name: 'Аккаунт', description: 'POST — регистрация и личные данные. GET — вход по email и паролю.', 'x-en': { description: 'POST for registration/profile; GET for email and password login.' } },
    ...
    paths: {
      // POST: без id создаётся аккаунт, с id сохраняется. GET проверяет вход.
      ...
      // студент видит форму регистрации/профиля и отдельный вход.
      '/api/users': {
        post: {
          tags: ['Аккаунт'],
          description: 'Без id — регистрация: fullName, email и password обязательны. С id — сохранение своего профиля с Bearer-токеном из входа. Фотография передаётся файлом avatar.',
          security: [{}, { bearerAuth: [] }],
          ...
          responses: Object.assign({
            '201': {
              ...
              content: { 'application/json': { schema: envelope('AccountSession') } }
            },
            '200': {
              description: 'Изменения сохранены',
              'x-en': { description: 'Changes saved' },
              content: { 'application/json': { schema: envelope('AccountUser') } }
            }
          }, badRequest, conflict, authErrors, tooLarge, badType, notFound, tooMany,
            errAt(503, 'Нужно настроить хранилище аккаунтов', 'Account storage needs configuration'))
        }
      },
      '/api/users/login': {
        get: {
          ...
          description: 'Нажмите замок / Authorize и заполните basicAuth: Username — email, Password — пароль регистрации. Пароль проверяется сервером и не возвращается. В data.user — сохранённые имя, фото и профиль; data.token нужен для сохранения изменений. В браузере пароль не передаётся в URL.',
          security: [{ basicAuth: [] }],
          responses: Object.assign(ok('AccountSession', 'Вход выполнен', 'Signed in'),
            errAt(401, 'Неверный email или пароль', 'Invalid email or password'), tooMany,
            errAt(503, 'Нужно настроить хранилище аккаунтов', 'Account storage needs configuration'))
        }
      },
      '/api/reviews': {
    ...
    components: {
      securitySchemes: {
        basicAuth: { type: 'http', scheme: 'basic', description: 'Вход: Username = email, Password = пароль' },
        bearerAuth: { type: 'http', scheme: 'bearer', bearerFormat: 'JWT', description: 'data.token из входа или регистрации — для изменения своего аккаунта' }
      },
      ...
      schemas: {
          ...
          properties: {
            code: {
              ...
              enum: ['VALIDATION_ERROR', 'UNAUTHORIZED', 'INVALID_TOKEN', 'TOKEN_REVOKED', 'INVALID_CREDENTIALS',
                ...
                'TOO_MANY_REQUESTS', 'ACCOUNT_STORAGE_UNAVAILABLE', 'INTERNAL_ERROR'],
              example: 'NOT_FOUND'
            },
            message: { type: 'string', example: 'Товар 999 не найден' },
            details: {
        ...
        AccountSession: {
          ...
          properties: {
            ...
            token: { type: 'string' },
            expiresIn: { type: 'integer', description: 'Срок токена в секундах' }
          }
        },
        AccountUser: {
          type: 'object',
          description: 'Свой профиль после проверки аккаунта. Пароля и хеша нет.',
          properties: {
            id: { type: 'integer' }, fullName: { type: 'string' },
            email: { type: 'string' }, tel: { type: 'string' },
            address: { type: 'string' }, avatar: { type: 'string' },
            role: { type: 'string' }, createdAt: { type: 'string' }, updatedAt: { type: 'string' }
          }
        },
        UserForm: {
          type: 'object',
