  ...
  res.end(JSON.stringify({
    ...
    info: {
      ...
      description:
        ...
        'Аккаунт держится на двух запросах — POST /api/users создаёт его и сохраняет изменения, ' +
        'DELETE /api/users/{id} удаляет. Проверок на форме нет: принимается любая почта, ' +
        'любой пароль и пустые поля. ' +
    ...
    tags: [
      { name: 'Users', description: 'Аккаунт: один POST создаёт и сохраняет, DELETE удаляет. Проверок нет — форма отправляется с первого раза.', 'x-en': { description: 'Account: one POST creates and saves, DELETE removes. No validation — the form goes through on the first try.' } },
    ...
    paths: {
      '/api/users': {
        post: {
          ...
          description: 'Один запрос на обе страницы. Без id — создаётся новый аккаунт (регистрация), ' +
            'с id — сохраняются изменения («Личные данные»). id можно прислать в теле запроса ' +
            'или в адресе: POST /api/users/{id} работает так же.\n\n' +
            'Тело — multipart/form-data: текстовые поля и файл avatar (кнопка «Choose File»). ' +
            'Проверок нет: любая почта, любой пароль, любое имя и пустые поля принимаются. ' +
            'Фото берётся размером с телефонный снимок (до 10 МБ) и сжимается сервером ' +
            'в квадрат 512×512 — на Vercel сама платформа не пропускает запрос больше 4.5 МБ.\n\n' +
            'В ответе личные данные закрыты: Ма*****, saf*****@gmail.com, +9929001*****, Душ*****. ' +
            'Форма «Личные данные» заполняется этими же значениями и присылает их обратно — ' +
            'такие поля сервер не записывает, оставляет прежние. Что пропущено, видно в unchanged.\n\n' +
            'Сохраните id из ответа: он нужен, чтобы потом сохранять изменения и удалить аккаунт.',
          requestBody: {
            required: false,
            content: {
              'multipart/form-data': {
      ...
      '/api/users/{id}': {
        delete: {
          ...
          'x-en': { summary: 'Delete an account' },
          description: 'Удаляет аккаунт вместе с фотографией. Тренажёр и сам держит только ' +
            'последние 50 регистраций: когда появляется новая, самая старая уходит.',
          parameters: [pathParam('id', 'id пользователя из ответа POST', 'User id from the POST response', 'integer')],
      ...
      '/api/users/{id}/avatar': {
        get: {
          tags: ['Users'],
          summary: 'Файл фотографии',
          'x-en': { summary: 'Photo file' },
          description: 'Отдаёт саму картинку, а не конверт — эту ссылку вставляют прямо в <img src>. ' +
    ...
    components: {
      schemas: {
        ...
        UserForm: {
          type: 'object',
          description: 'Форма регистрации и «Личных данных». Отправляется как multipart/form-data. ' +
