  ...
  res.end(JSON.stringify({
    ...
    info: {
      ...
      description:
        ...
        'Это учебный тренажёр: читать и менять данные можно без токена и без входа. ' +
        'Проверок на форме регистрации нет — принимается любая почта, любой пароль и пустые поля. ' +
        'Личные данные наружу уходят закрытыми: Ма*****, saf*****@gmail.com, +9929001*****. ' +
      ...
      'x-en': {
        ...
        description:
          ...
          'A practice sandbox: reading and writing need no token and no login. ' +
          'Personal data comes back masked. Every response comes as { data, errors, statusCode }: ' +
    ...
    tags: [
      { name: 'Authentication', description: 'Регистрация с фотографией и вход. Проверок нет — форма отправляется с первого раза.', 'x-en': { description: 'Sign up with a photo and log in. No validation — the form goes through on the first try.' } },
      { name: 'Users', description: 'Аккаунты и страница «Личные данные»: загрузить профиль, сохранить изменения, поменять фото', 'x-en': { description: 'Accounts and the personal-data page: load a profile, save changes, replace the photo' } },
    ...
    paths: {
      '/api/auth/register': {
        post: {
          tags: ['Authentication'],
          summary: 'Регистрация: поля формы + фото',
          'x-en': { summary: 'Register: form fields + photo' },
          description: 'Тело отправляется как multipart/form-data: текстовые поля и файл avatar ' +
            '(кнопка «Choose File»). Проверок нет — принимается любая почта, любой пароль и любое ' +
            'имя, пустые поля тоже: это тренажёр, форма должна отправляться с первого раза. ' +
            'Фото принимается размером с телефонный снимок (до 10 МБ) и сжимается сервером ' +
            'в квадрат 512×512 — на Vercel сама платформа не пропускает запрос больше 4.5 МБ. ' +
            'В ответе личные данные приходят закрытыми: Ма*****, saf*****@gmail.com, +9929001*****. ' +
            'То же самое можно отправить обычным JSON, тогда фото просто не будет.',
          requestBody: {
            required: false,
            content: {
              'multipart/form-data': {
          ...
          responses: Object.assign({
            '201': {
              ...
              content: { 'application/json': { schema: envelope('AuthUser') } }
            }
          }, tooLarge, badType, tooMany)
        }
      },
      '/api/auth/login': {
        post: {
          ...
          'x-en': { summary: 'Log in with email and password' },
          description: 'Находит аккаунт по email и проверяет пароль. Если при регистрации пароль ' +
            'не задавали, вход свободный. В ответе тот же объект пользователя, что и у регистрации, ' +
            'плюс accessToken — он нужен только в строгом режиме, обычные запросы работают без него.',
          requestBody: body('LoginInput', { email: 'safarov@gmail.com', password: 'ExamplePassword123!' }),
          responses: Object.assign(
            ok('AuthUser', 'Вход выполнен', 'Logged in'),
            errAt(401, 'Пользователь не найден или пароль неверный', 'User not found or wrong password'), tooMany)
        }
      },
      '/api/users': {
        get: {
          tags: ['Users'],
          summary: 'Все учебные аккаунты',
          'x-en': { summary: 'All practice accounts' },
          description: 'Тренажёр хранит последние 50 регистраций — когда их становится больше, ' +
            'самые старые удаляются сами. Личные поля закрыты звёздочками.',
          parameters: [
            q('search', 'string', 'Поиск по имени и email', 'Search by name and email'),
            q('role', 'string', 'user или admin', 'user or admin', { enum: ['user', 'admin'] }),
            ...PAGING
          ],
          responses: ok('UserPage', 'Страница аккаунтов', 'A page of accounts')
        }
      },
      '/api/users/{id}': {
        get: {
          tags: ['Users'], summary: 'Один аккаунт', 'x-en': { summary: 'A single account' },
          description: 'Этим ответом заполняют форму «Личные данные». Значения приходят ' +
            'закрытыми: Ма*****, +9929001*****.',
          parameters: [pathParam('id', 'id пользователя', 'User id', 'integer')],
          responses: Object.assign(ok('User', 'Аккаунт найден', 'Account found'), notFound)
        },
        patch: {
          tags: ['Users'], summary: 'Сохранить изменения', 'x-en': { summary: 'Save changes' },
          description: 'Кнопка «Сохранить изменения» на странице «Личные данные». Тело — ' +
            'multipart/form-data (можно и JSON): имя, телефон, адрес и новое фото. ' +
            'Меняется только то, что прислали. Значение со звёздочками сервер не записывает, ' +
            'а оставляет прежнее — форма ведь заполнена закрытыми значениями, и возвращать ' +
            'их обратно нормально. Какие поля пропущены, видно в поле unchanged.',
          parameters: [pathParam('id', 'id пользователя', 'User id', 'integer')],
          requestBody: {
            required: false,
            content: {
              ...
              'application/json': {
                ...
                example: { fullName: 'Мансур Сафаров', tel: '+992900123456', address: 'Душанбе, Сино' }
              }
            }
          },
          responses: Object.assign(ok('User', 'Сохранено', 'Saved'), tooLarge, badType, notFound)
        },
        put: {
          tags: ['Users'], summary: 'То же самое методом PUT', 'x-en': { summary: 'The same via PUT' },
          description: 'Работает как PATCH — на случай, если во фронтенде привычнее PUT.',
          parameters: [pathParam('id', 'id пользователя', 'User id', 'integer')],
          requestBody: {
            required: false,
            content: {
              'multipart/form-data': { schema: { $ref: '#/components/schemas/UserForm' } },
        ...
        delete: {
          tags: ['Users'], summary: 'Удалить аккаунт', 'x-en': { summary: 'Delete an account' },
          parameters: [pathParam('id', 'id пользователя', 'User id', 'integer')],
          responses: Object.assign(ok(null, 'Аккаунт удалён', 'Account deleted'), notFound)
        }
      },
      '/api/users/{id}/avatar': {
        get: {
          tags: ['Users'], summary: 'Файл фото', 'x-en': { summary: 'Photo file' },
          description: 'Отдаёт саму картинку, а не конверт — ссылку вставляют прямо в <img src>. ' +
            'Всегда квадрат 512×512 JPEG, поэтому круглая маска в профиле не срезает лицо.',
          parameters: [pathParam('id', 'id пользователя', 'User id', 'integer')],
          responses: Object.assign({
            '200': {
    ...
    components: {
      schemas: {
        ...
        User: {
          ...
          properties: {
            ...
            unchanged: {
              ...
              description: 'Появляется после сохранения: поля, которые пришли закрытыми и потому не записаны'
            }
          }
        },
        AuthUser: {
          allOf: [
            { $ref: '#/components/schemas/User' },
            {
              type: 'object',
              properties: {
                accessToken: { type: 'string', description: 'Нужен только в строгом режиме ADMIN_WRITES' },
        ...
        UserForm: {
          type: 'object',
          description: 'Форма «Личные данные». Отправляется как multipart/form-data.',
          properties: {
            fullName: { type: 'string', example: 'Мансур Сафаров' },
            tel: { type: 'string', example: '+992900123456' },
            address: { type: 'string', example: 'Москва, ул. Московская 25-45' },
            password: { type: 'string', format: 'password' },
            avatar: { type: 'string', format: 'binary', description: 'Новое фото, до 10 МБ' }
          }
        },
        UserPatch: {
          type: 'object',
          properties: {
            fullName: { type: 'string' }, tel: { type: 'string' },
            address: { type: 'string' }, password: { type: 'string', format: 'password' }
          }
        },
        RegisterForm: {
          type: 'object',
          description: 'Форма регистрации: текстовые поля и файл. Обязательных полей нет.',
          properties: {
            ...
            avatar: { type: 'string', format: 'binary', description: 'Фото до 10 МБ — сервер сожмёт его сам' }
          }
        },
        RegisterInput: {
          type: 'object',
          description: 'То же самое обычным JSON — тогда фото не загружается',
          properties: {
